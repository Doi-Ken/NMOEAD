...
void dmp_twovar(double y[], double fit[], int ob, int k){
	double MAX_LIMIT = 50.0;
	double MIN_LIMIT = -50.0;
	double r = 25;
	...
	double **A = new double*[ob];
	for (int o = 0; o < ob; o++){
		A[o] = new double[2];
	}
	for (int o = 0; o < ob; o++){
		A[o][0] = -1.0 * sin(2.0 * M_PI * o / ob) * r + c[0];
		A[o][1] = cos(2.0 * M_PI * o / ob) * r + c[1];
	}

	for (int i = 0; i < k; i++){
		Y[i] = (MAX_LIMIT - MIN_LIMIT) * y[i] - (MAX_LIMIT - MIN_LIMIT) / 2.0;
	}

	for (int o = 0; o < ob; o++){
		double distance = 0.0;
		for (int i = 0; i < k; i++){
			distance += (Y[i] - A[o][i]) * (Y[i] - A[o][i]);
		}
...
void dmp(double y[], double fit[], int ob, int k){
	double MAX_LIMIT = 50.0;
	double MIN_LIMIT = -50.0;
	double r = 25;
	double *c = new double[2];
...
void pdmp(double y[], double fit[], int ob, int k){
	double MAX_LIMIT = 50.0;
	double MIN_LIMIT = -50.0;
	double r = 10;
	int polysize = 4;
	double **c = new double*[polysize];

	double *Y = new double[k];
	double ***A = new double**[polysize];
	double ***B = new double**[polysize];
	...
	for (int p = 0; p < polysize; p++){
		for (int o = 0; o < ob; o++){
			A[p][o][0] = -1.0 * sin(2.0 * M_PI * o / ob) * r + c[p][0];
			A[p][o][1] = cos(2.0 * M_PI * o / ob) * r + c[p][1];
		}
	}
	...
	double *min_dis = new double[ob];
	for (int o = 0; o < ob; o++){
		min_dis[o] = 1000000000.0;
	}
	for (int p = 0; p < polysize; p++){
		for (int o = 0; o < ob; o++){
			double distance = 0.0;
			for (int i = 0; i < k; i++){
				distance += (Y[i] - B[p][o][i]) * (Y[i] - B[p][o][i]);
			}
			distance = sqrt(distance);
...
void maxproblem(double y[], double fit[], char problem[], int ob, int k, int l, int k_dtlz, double **P, int point_num){

	if (strcmp("maxdtlz1", problem) == 0){
		dtlz1(y, fit, ob, k_dtlz);
		//std::cout << "dtlz1" << std::endl;
	}
	else if (strcmp("maxdtlz2", problem) == 0){
		dtlz2(y, fit, ob, k_dtlz);
		//std::cout << "dtlz2" << std::endl;
	}
	else if (strcmp("maxdtlz3", problem) == 0){
		dtlz3(y, fit, ob, k_dtlz);
		//std::cout << "dtlz3" << std::endl;
	}
	else if (strcmp("maxdtlz4", problem) == 0){
		dtlz4(y, fit, ob, k_dtlz);
		//std::cout << "dtlz4" << std::endl;
	}
	else{
		if (strcmp("maxwfg1", problem) == 0){
			hoge(y, fit, "wfg1", ob, k, l);
		}else if (strcmp("maxwfg2", problem) == 0){
			hoge(y, fit, "wfg2", ob, k, l);
		}else if (strcmp("maxwfg3", problem) == 0){
			hoge(y, fit, "wfg3", ob, k, l);
		}
		else if (strcmp("maxwfg4", problem) == 0){
			hoge(y, fit, "wfg4", ob, k, l);
		}
		else if (strcmp("maxwfg5", problem) == 0){
			hoge(y, fit, "wfg5", ob, k, l);
		}
		else if (strcmp("maxwfg6", problem) == 0){
			hoge(y, fit, "wfg6", ob, k, l);
		}
		else if (strcmp("maxwfg7", problem) == 0){
			hoge(y, fit, "wfg7", ob, k, l);
		}
		else if (strcmp("maxwfg8", problem) == 0){
			hoge(y, fit, "wfg8", ob, k, l);
		}
		else if (strcmp("maxwfg9", problem) == 0){
			hoge(y, fit, "wfg9", ob, k, l);
		}
...
void inverted_dtlz1(double y[], double fit[], int ob, int k){
	int x_m = k;
	double g_x_m = 0.0;
	...
	for (int i = 0; i < ob; i++){
		fit[i] = 0.5 * (1.0 + g_x_m) - fit[i];
	}
}



void inverted_dtlz2(double y[], double fit[], int ob, int k){
	int x_m = k;
	double g_x_m = 0.0;
	...
	for (int i = 0; i < ob; i++){
		fit[i] = (1.0 + g_x_m) - fit[i];
	}
}

void zdt1(double y[], double fit[], int ob, int k){
	if (ob != 2){
		cout << "objective number error" << endl;
		exit(1);
	}
...
void zdt2(double y[], double fit[], int ob, int k){
	if (ob != 2){
		cout << "objective number error" << endl;
		exit(1);
	}
...
void zdt3(double y[], double fit[], int ob, int k){
	if (ob != 2){
		cout << "objective number error" << endl;
		exit(1);
	}
...
void zdt4(double y[], double fit[], int ob, int k){
	if (ob != 2){
		cout << "objective number error" << endl;
		exit(1);
	}
...
void fitness_knap(int y[], double fit[], int item, int ob, double alpha){
	...
	for (int o = 0; o < ob; o++){
			...
			if(o % 2 == 0){
				fit[o] = alpha * fit[0] + (1.0 - alpha) * sum_profit[o];
			}
			if(o % 2 != 0){
				fit[o] = alpha * fit[1] + (1.0 - alpha) * sum_profit[o];
			}
		}
	}
	delete[] sum_weight;
...
void minus_fitness_knap(int y[], double fit[], int item, int ob, double alpha){
	double *sum_profit = new double[ob];
	double *sum_weight = new double[ob];
...
void check_input_file_output(int y[], int item, int ob){
	...
	fout_file << endl << endl;
	fitness_knap(y, fit, item, ob, 0.0);
...
void mtest_type_dtlz(double y[], double fit[], int ob, int k, double **P, int point_num){
	int k_point = (int)floor(4.0 * y[0]);
	int k_point_plus1 = k_point + 1;
	if (y[0] == 1.0){
		k_point = point_num - 2;
	}
	k_point_plus1 = k_point + 1;

	double t = 4.0 * y[0] - k_point;

	double *Vk = new double[ob];
	double *Vk_plus1 = new double[ob];


	if (k_point == 0){
		for (int o = 0; o < ob; o++){
			Vk[o] = (P[1][o] - P[0][o]) / 2.0;
		}
		for (int o = 0; o < ob; o++){
			Vk_plus1[o] = (P[k_point_plus1 + 1][o] - P[k_point_plus1 - 1][o]) / 2.0;
		}

	}
	else if (k_point_plus1 == 4){

		for (int o = 0; o < ob; o++){
			Vk[o] = (P[k_point + 1][o] - P[k_point - 1][o]) / 2.0;
		}
		for (int o = 0; o < ob; o++){
			Vk_plus1[o] = (P[point_num - 1][o] - P[point_num - 2][o]) / 2.0;
		}
	}
	else{
		for (int o = 0; o < ob; o++){
	...
	for (int o = 0; o < ob; o++){
		fit[o] = (2.0 * t + 1.0) * (1.0 - t) * (1.0 - t)*P[k_point][o]
			+ t * (1.0 - t) * (1.0 - t) * Vk[o]
			+ t * t * (3.0 - 2.0 * t)*P[k_point + 1][o]
			+ t * t * (t - 1.0) * Vk_plus1[o];
	}

	double g = 0.0;
	for (int n = 1; n < k; n++){
		g += (2.0 * y[n] - 1.0) * (2.0 * y[n] - 1.0);
	}
	g *= 5.0;
	g = g / ((double)k - 1.0);
...
void mtest_type_wfg(double y[], double fit[], int ob, int k, double **P, int point_num){
	int k_point = (int)floor(4.0 * y[0]);
	int k_point_plus1 = k_point + 1;
	if (y[0] == 1.0){
		k_point = point_num - 2;
	...
	double t = 4.0 * y[0] - (double)k_point;
...
void test20160301(int y[], double fit[], int ob, int k){
	int x_m = k;
	double g_x_m = 0.0;
...
void test20160303(int y[], double fit[], int ob, int k, int m){
...
void  fonseca(double y[], double fit[], int ob, int k){
	y[0] = 8.0 * (y[0] - 0.5);
	y[1] = 8.0 * (y[1] - 0.5);
	fit[0] = 1.0 - exp(-1.0 * (y[0] - 1.0) * (y[0] - 1.0) - (y[1] + 1.0) * (y[1] + 1.0));
	fit[1] = 1.0 - exp(-1.0 * (y[0] + 1.0) * (y[0] + 1.0) - (y[1] - 1.0) * (y[1] - 1.0));

	
}

void fitness(double y[], double fit[], char problem[], int ob, int k, int l,int k_dtlz, double **P, int point_num){
	...
	else if (strcmp("inverted_dtlz1", problem) == 0){
		inverted_dtlz1(y, fit, ob, k_dtlz);
		//std::cout << "dtlz2" << std::endl;
	}
	else if (strcmp("inverted_dtlz2", problem) == 0){
		inverted_dtlz2(y, fit, ob, k_dtlz);
		//std::cout << "dtlz2" << std::endl;
	}

	else if (strcmp("zdt1", problem) == 0){
		zdt1(y, fit, ob, k_dtlz);
		//std::cout << "dtlz1" << std::endl;
	}
	else if (strcmp("zdt2", problem) == 0){
		zdt2(y, fit, ob, k_dtlz);
		//std::cout << "dtlz2" << std::endl;
	}
	else if (strcmp("zdt3", problem) == 0){
		zdt3(y, fit, ob, k_dtlz);
		//std::cout << "dtlz3" << std::endl;
	}
	else if (strcmp("zdt4", problem) == 0){
		zdt4(y, fit, ob, k_dtlz);
		//std::cout << "dtlz4" << std::endl;
	}
	else if (strcmp("mtest_dtlz", problem) == 0){
		mtest_type_dtlz(y, fit, ob, k_dtlz, P, point_num);
	}
	else if (strcmp("mtest_wfg", problem) == 0){
		mtest_type_wfg(y, fit, ob, k_dtlz, P, point_num);
	}
	else if (strcmp("fonseca", problem) == 0){
		fonseca(y, fit, ob, k_dtlz);
		//std::cout << "dtlz1" << std::endl;
	}
	else if (strcmp("dmp_twovar", problem) == 0){
		dmp_twovar(y, fit, ob, k_dtlz);
	}
	else if (strcmp("dmp", problem) == 0){
		dmp(y, fit, ob, k_dtlz);
	}
	else if (strcmp("pdmp", problem) == 0){
		pdmp(y, fit, ob, k_dtlz);
	}
