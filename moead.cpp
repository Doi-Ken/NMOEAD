...
double AchivementScalarizingFunction(double *fit, double *lambda, double *nadir, double *z, int ob){
	double max = 1.0 / lambda[0] * (double)fabs((fit[0] - z[0]) / (nadir[0] - z[0]));
	for (int o = 0; o < ob; o++){
		double g = 1.0 / lambda[o] * (double)fabs((fit[o] - z[o]) / (nadir[o] - z[o]));
		if (max < g){
			max = g;
		}
	}
...
void CalcInterception(int matrixsize, double **e, double *z, double *nadir, double *a){

	MatrixXd E = MatrixXd::Random(matrixsize, matrixsize);
	for (int i = 0; i < matrixsize; i++){
		for (int j = 0; j < matrixsize; j++){
			E(i, j) = e[i][j] - z[j];
		}
	}
	FullPivLU< MatrixXd > lu(E);
	
	if (lu.rank() != matrixsize){
		for (int i = 0; i < matrixsize; i++){
			a[i] = nadir[i];
		}
	}
		...
		//	<< E*B << std::endl << std::endl;

		double *e_sum = new double[matrixsize];
		for (int i = 0; i < matrixsize; i++){
			e_sum[i] = 0.0;
			for (int j = 0; j < matrixsize; j++){
				e_sum[i] += B(i, j);
...
void CalcExtremePoint(double **e, double **fit, double *nadir, double *z, int ob, int POPSIZE){

	double *lambda = new double[ob];
	...
	for (int o = 0; o < ob; o++){
		...
		lambda[o] = 1.0;
		double min = AchivementScalarizingFunction(fit[0], lambda, nadir, z, ob);
		double AFvalue = min;
		int AFindex = 0;
		for (int i = 0; i < POPSIZE; i++){

			AFvalue = AchivementScalarizingFunction(fit[i], lambda, nadir, z, ob);
			if (AFvalue < min){
				min = AFvalue;
				AFindex = i;
			}
...
double g_t_e(double *fit, double *lambda, double *z, int ob){
	double max = (lambda[0] == 0) ? 0.000001 * (double)fabs(z[0] - fit[0]) : lambda[0] * (double)fabs(z[0] - fit[0]);
	for (int o = 0; o < ob; o++){
		double g = (lambda[o] == 0) ? 0.000001 * (double)fabs(z[o] - fit[o]) : lambda[o] * (double)fabs(z[o] - fit[o]);
		if (max < g){
			max = g;
		}
	}
...
//double g_t_e(double *fit, double *lambda, double *z, int ob){
//	double max = (lambda[0] == 0) ? 1.0 / 0.000001 * (double)fabs(z[0] - fit[0]) : 1.0 / lambda[0] * (double)fabs(z[0] - fit[0]);
//	for (int o = 0; o < ob; o++){
//		double g = (lambda[o] == 0) ? 1.0 / 0.000001 * (double)fabs(z[o] - fit[o]) : 1.0 / lambda[o] * (double)fabs(z[o] - fit[o]);
//		if (max < g){
//			max = g;
//		}
//	}
